...
def calcVarianceAndStandarDeviation(table, n, x):
    ...
    sumFixi1 = sum(item["fi.xi"] for item in table)
    print(sumFixi1)
...
def main():
    ...
    while True:
        ...
        if option == '1':
            ...
            print(f"Índice de Asimetría: {round(asymmetryIndex, 4) if not math.isnan(asymmetryIndex) else 'No calculable'}")
            print(f"Distribución: {typeA}")

        elif option == '12':
            print(f"Coeficiente de variación: {round(cv, 4) if not math.isnan(asymmetryIndex) else 'No calculable'}")
            print(f"%: {round(cv*100, 2) if not math.isnan(asymmetryIndex) else 'No calculable'}")
